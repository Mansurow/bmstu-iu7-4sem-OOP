...
Point DrawCarcassModelAdapter::getProjection(const Point &point)
{
...
void DrawCarcassModelAdapter::request()
{
    if (_adaptee && _camera && _drawer)
    {
        ...
        for (auto edge : edges)
        {
            _drawer->drawLine(getProjection(points.at(edge.getFirst() - 1)).getAbsVertex(center),
                              getProjection(points.at(edge.getSecond() - 1)).getAbsVertex(center));
        }
    }
}
